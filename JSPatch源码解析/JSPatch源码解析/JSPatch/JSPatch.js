...
;(function() {
  ...
  var _methodFunc = function(instance, clsName, methodName, args, isSuper, isPerformSelector) {
    ...
    // 最后对象方法调用的是在native里注册了的_OC_callI
    // 类方法调用的是在native里注册了的_OC_callC方法
    // 这两个方法都走的是callSelector方法
  ...
  var _require = function(clsName) {
    if (!global[clsName]) {
      // 在JS全局作用域上创建一个同名变量
      global[clsName] = {
        // 变量指向一个对象，对象属性__isCls表明这是一个Class
        __isCls: 1,
  ...
  global.defineClass = function(declaration, properties, instMethods, clsMethods) {
    ...
    // 调用了我们之前注册的_OC_defineClass方法，传递进去了类名、实例方法、类方法的实现
