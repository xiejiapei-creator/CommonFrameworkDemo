...
static NSDictionary *defineClass(NSString *classDeclaration, JSValue *instanceMethods, JSValue *classMethods)
{
    ...
    // 格式 xxObject : xxSuperObject <xxProtocol>
    // 解析类
    ...
    for (int i = 0; i < 2; i ++) {
        ...
        for (NSString *jsMethodName in methodDict.allKeys) {
            ...
            if (class_respondsToSelector(currCls, NSSelectorFromString(selectorName))) {// 如果已经实现了这个方法
...
// 一：解析出invocation的方法编码
    ...
    if (!jsFunc) {// 如果没有实现JS的方法就调用原始的消息转发
...
// 四：[jsFunc callWithArguments:params] 把解析出来的参数传递给JS方法
...
// 五：根据返回参数的编码，解析出返回值，赋值给invocation
        case '{': {
...
static void overrideMethod(Class cls, NSString *selectorName, JSValue *function, BOOL isClassMethod, const char *typeDescription)
{
    ...
    // 如果实现了这个方法，做一下区分生成一个新的 ORIGSelector -> 原始方法的IMP，目的是为了JS能调用到之前的方法
...
// 三：生成一个NSInvocation，并且设置target和方法编码
...
// 四：遍历方法编码，从传递过来的参数列表里拿到对应的参数赋值给NSInvocation
...
// 五：[invocation invoke]调用方法，根据返回值编码拿到返回值返回给JS
